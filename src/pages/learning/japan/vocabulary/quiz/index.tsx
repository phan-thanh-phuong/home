...
import { loadMastered, recordMastered, clearMastered } from '../mastered';
...
export const LearningJapanVocabularyQuiz = () => {
  ...
  useEffect(() => {
    if (!category || allVocabulary.length < 2) {
      navigate('/learning/japan/vocabulary');
    }
  ...
  const [masteredIds, setMasteredIds] = useState<string[]>(() =>
    level ? loadMastered(level) : [],
  );
  const [mistakeIds, setMistakeIds] = useState<string[]>(() =>
    level ? loadMistakes(level) : [],
  );

  const scopeTotal = pool.length;
  const scopeMastered = pool.filter((item) =>
  ...
  const finishAnswer = (correct: boolean) => {
    ...
    if (level && currentQuestion) {
      if (correct) {
        setMasteredIds(recordMastered(level, currentQuestion.target.id));
      }
  ...
  useEffect(() => {
    ...
    if (filteredPool.length < 2) {
      ...
      navigate(`/learning/japan/vocabulary/${level}`);
      return;
    }
    setPool(filteredPool);
    setQuestions(buildQuestions(filteredPool, choiceCount, direction));
    ...
    <>
      ...
      <div className="row mt-2 align-items-center">
        ...
        <div className="col-12 col-md-4 text-md-end">
          <p className="text-muted mb-1">
            Tiến độ bài đã chọn: {scopeMastered}/{scopeTotal} từ đã thuộc{' '}
            <button
              type="button"
              className="btn btn-outline-danger btn-sm"
              disabled={scopeMastered === 0}
      ...
      <div className="row mt-2">
        <div className="col-12">
          <div className="card text-white bg-dark mb-3">
            <div className="card-body text-center position-relative px-5">
              ...
              {currentQuestion.direction === 'ja-vi' && (
                <button
                  type="button"
                  className="btn btn-sm btn-light position-absolute top-50 end-0 translate-middle-y me-1 py-0 px-1"
                  title="Nghe câu hỏi"
                  onClick={() => speak(currentQuestion.target.hiragana)}
                >
                  🔊
                </button>
              )}
          ...
          {currentQuestion.choices.map((choice) => {
            ...
            return (
              <div key={choice.id} className="col-6 col-md-3">
                ...
                >
                  <div className="card-body text-center padding-custom position-relative px-4">
                    <p className="mb-0">
                      {currentQuestion.direction === 'ja-vi'
                        ? choice.translate
                        : renderJapanese(choice)}
                    </p>
                    {currentQuestion.direction === 'vi-ja' && (
                      <button
                        type="button"
                        className="btn btn-sm btn-light position-absolute top-50 end-0 translate-middle-y me-1 py-0 px-1"
                        title="Nghe đáp án"
