...
export const LearningJapanKanjiQuiz = () => {
  ...
  const [masteredIds, setMasteredIds] = useState<number[]>(() =>
    categoryId ? loadMastered(categoryId) : [],
  );
  ...
  const finishAnswer = (correct: boolean) => {
    ...
    if (categoryId && currentQuestion) {
      if (correct) {
        setMasteredIds(recordMastered(categoryId, currentQuestion.target.id));
      }
      setMistakeIds(
  ...
  useEffect(() => {
    ...
    if (filteredPool.length < 2) {
      ...
      navigate(`/learning/japan/kanji/${categoryId}`);
      return;
    }
    setPool(filteredPool);
    setQuestions(
    ...
    <>
      ...
      <div className="row mt-2 align-items-center">
        ...
        <div className="col-12 col-md-4 text-md-end">
          <p className="text-muted mb-1">
            Tiến độ bài đã chọn: {scopeMastered}/{scopeTotal} đã thuộc{' '}
            <button
              type="button"
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          speak(
                            currentQuestion.type === 'meaning-to-char'
                              ? choice.char
                              : choice.readingText || choice.char,
                          );
                        }}
                      >
                        🔊
